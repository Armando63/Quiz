...
namespace Quiz
{
	public partial class Preguntas : Form
	{
		...
		private int opcionAudioActual = -1;
		...
		private Thread hiloEscuchaServidor;
		...
		public Preguntas(int categoriaId)
		{
			...
			this.nombreJugador = Inicio.nombreJugadorGlobal;
			this.esMultijugador = false;
			ConfigurarFormulario();
		}

		public Preguntas(string categoria, string nombreJugador, TcpClient clienteExistente = null)
		{
			InitializeComponent();
			this.categoriaId = ObtenerIdCategoria(categoria);
			this.nombreJugador = nombreJugador;
			this.esMultijugador = true;

			if (clienteExistente != null)
			{
				this.clienteServidor = clienteExistente;
				this.streamServidor = clienteExistente.GetStream();
		...
		private void ConfigurarFormulario()
		{
			...
			this.FormClosing += Preguntas_FormClosing;
			...
			lblEstado = new Label()
			{
				...
				Width = 300,
				Height = 40,
				Location = new Point((this.Width - 300) / 2, (this.Height - 40) / 2),
		...
		{
			hiloEscuchaServidor = new Thread(EscucharServidor);
			hiloEscuchaServidor.IsBackground = true;
		...
		private void Preguntas_Resize(object sender, EventArgs e)
		{
			...
			if (lblEstado != null)
			{
				lblEstado.Location = new Point((this.Width - 300) / 2, (this.Height - 40) / 2);
			}
			Invalidate();
		}
		...
		private void Preguntas_Paint(object sender, PaintEventArgs e)
		{
			...
			string textoNumero = esMultijugador ?
				$"Pregunta {indiceActual + 1} | {nombreJugador}" :
				$"Pregunta {indiceActual + 1} de {apiPreguntas.Count}";
		...
		private void EvaluarRespuesta(int index, Opcion opcion)
		{
			...
			bool esCorrecta = opcion.EsCorrecta;
			...
			EnviarRespuestaAlServidor(index, esCorrecta);
		...
		private void FinalizarQuiz()
		{
			...
			{
				lblEstado.Text = "Esperando resultados finales...";
				lblEstado.Visible = true;
			}
		}

		private void FinalizarQuizMultijugador(dynamic puntajes)
		{
			reproductor.controls.stop();
